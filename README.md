...
|Abstract Factory|`BaseNodeFactory` in `src/compiler/factory/baseNodeFactory.ts`|TypeScript needs to able to createa a syntax tree for parsing JavaScript, which is composed of nodes of a variety of predefined types representing different parts of syntax.|To abstract out this creation, TypeScript uses a `BaseNodeFactory` to abstract out creating a group of nodes. This way, clients of this class don't need to worry about the nitty-gritty details of all of the `ObjectAllocator`'s `Node` constructors and can create instances of nodes more simply and within a shared context.|
